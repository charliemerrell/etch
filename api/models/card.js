...
function addCard(userId, question, answer) {
  return db.none(`INSERT INTO cards
...
async function cardBelongsToUser(cardId, userId) {
  const rows = await db.one('SELECT COUNT(*) FROM cards WHERE id = $1 AND user_id = $2', [cardId, userId]);
  return rows.count === '1';
}
...
async function handleAnswer(cardId, correct) {
  const card = await db.one('SELECT progress FROM cards WHERE id = $1', [cardId]);
  ...
  db.none('UPDATE cards SET progress = $1, next_answer_after = $2 WHERE id = $3',
